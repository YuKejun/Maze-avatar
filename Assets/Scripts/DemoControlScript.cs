...
public class DemoControlScript : MonoBehaviour
{
		...
		void Update ()
		{
				...
				if (is_walking) {
						if (Vector3.Distance (last_position, transform.position) >= grid_size) {
								...
								if (Input.GetAxis ("Vertical") <= 0) {
										...
										walking_dest = transform.position + transform.rotation * Vector3.forward * grid_size;
								}
						} else {
				...
				if (!is_walking && Input.GetAxis ("Vertical") > 0) {
						is_walking = true;
				...
				if (Input.GetKey ("q") && (myAnimator.GetInteger ("CurrentAction") == 0)) {
						...
						if ((is_walking == false) && (Input.GetAxis ("Horizontal") == 0)) {
